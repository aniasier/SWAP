...
PURE RECURSIVE COMPLEX*16 FUNCTION single_electron_expected_value(Psi1, Psi2, norbs, Nx, ham_1_size)
  IMPLICIT NONE
  COMPLEX*16, INTENT(IN) :: Psi1(ham_1_size), Psi2(ham_1_size)
  INTEGER*4, INTENT(IN) :: ham_1_size
  INTEGER*4, INTENT(IN) :: norbs, Nx
  INTEGER*4 :: i
  single_electron_expected_value = (0.0d0, 0.0d0)
  DO i = 1, ham_1_size
    single_electron_expected_value = single_electron_expected_value + CONJG(Psi1(i))*Psi2(i)
  END DO
  RETURN
END FUNCTION single_electron_expected_value
...
PURE RECURSIVE COMPLEX*16 FUNCTION scalar_product(Psi1, Psi2, ham_1_size)
    IMPLICIT NONE
    COMPLEX*16, INTENT(IN) :: Psi1(ham_1_size), Psi2(ham_1_size)
    INTEGER*4, INTENT(IN) :: ham_1_size
    INTEGER*4 :: i

    scalar_product = DCMPLX(0.0d0, 0.0d0)
  
    DO i = 1, ham_1_size
      scalar_product = scalar_product + CONJG(Psi1(i)) * Psi2(i)
    END DO
