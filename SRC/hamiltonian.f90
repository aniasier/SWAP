...
CONTAINS
  ...
  SUBROUTINE WAVEFUNCTION_CROSSSECTION(Psi_LR, ham_1_size,  nstate_1_noSO, x0, y0, Psi_cross)
    ...
    COMPLEX*16, INTENT(IN) :: Psi_LR(ham_1_size, nstate_1_noSO)
    INTEGER*4, INTENT(IN) :: ham_1_size, nstate_1_noSO
    INTEGER*4, INTENT(IN) :: x0, y0
    COMPLEX*16, INTENT(OUT) :: Psi_cross(ham_1_size,1)
    INTEGER*4 :: i, j, im, n, psi_indx, mesh_size, iorb, indx
    ...
    DO j = -ny, ny
      ...
      END DO
    END DO
    psi_indx = (Ordering(x0,y0)-1)

    mesh_size = (2*Nx + 1) * (2*Ny + 1)
    DO im = 0, mesh_size-1
      DO iorb = 1, norbs
        indx = im*norbs + iorb
        Psi_cross(indx,1) = Psi_LR(indx, 1) * Psi_LR(psi_indx + iorb, 2) - Psi_LR(psi_indx + iorb, 1) * Psi_LR(indx, 2) 
      END DO
  ...
  !   COMPLEX*16, INTENT(IN) :: Cm(nstates_2)
  !   INTEGER*4, INTENT(IN) :: ham_1_size
  !   INTEGER*4, INTENT(IN) :: nstates_1,nstates_2, nmax, t_max_int, nx, ny, norbs
  !   REAL*8, INTENT(IN) :: Energies_1(nstates_1)
  !   REAL*8, INTENT(IN) :: dt 
  !   INTEGER*4 :: ti, m 
  !   REAL*8 :: t
  !   COMPLEX*16 :: sz_R, sz_L

  !   COMPLEX*16, INTENT(OUT) :: Spin_t(t_max_int, nmax)
  !   m = 1
  !   Spin_t(:,:) = 0.0
  !   sz_R = sigma_z_expected_value_R(Psi_1(:,m), Psi_1(:,m), ham_1_size, nx, ny, norbs)
  !   sz_L = sigma_z_expected_value_L(Psi_1(:,m), Psi_1(:,m), ham_1_size, nx, ny, norbs)

  !   DO ti = 1, t_max_int
  !     t = (ti-1) * dt
  !     Spin_t(ti, 1) = sz_L * Cm(m) * EXP(-imag * (Energies_1(m)) * t)
  !     Spin_t(ti, 2) = sz_R * Cm(m) * EXP(-imag * (Energies_1(m)) * t)
  !   END DO
  ! END SUBROUTINE TIME_EVOLUTION_SINGLE_SPIN_EXPECTATION
