...
SUBROUTINE WRITE_MULTI_ELECTRON_DENSITY(Particle_density, psi_size, nstates, Nx, Ny, dx, filename)
  IMPLICIT NONE
  COMPLEX*16, INTENT(IN) :: Particle_density(psi_size, nstates)
  INTEGER*4, INTENT(IN) :: Nx, Ny, psi_size, nstates
  REAL*8, INTENT(IN) :: dx
  CHARACTER(LEN=*), INTENT(IN) :: filename
  CHARACTER(LEN=200) :: filename_state

  INTEGER*4 :: nn, ix, iy, is
  ...
  DO is = 1, nstates
    ...
    DO ix = -Nx, Nx
      WRITE(1,'(2E20.8)', ADVANCE='NO') ix*dx/nm2au, iy*dx/nm2au
      WRITE(1, '(2E20.8)', ADVANCE='NO') REAL(Particle_density(nn,is)), AIMAG(Particle_density(nn,is))
      nn = nn + 1
      WRITE(1,*)
    END DO
    END DO
...
END SUBROUTINE WRITE_MULTI_ELECTRON_DENSITY
